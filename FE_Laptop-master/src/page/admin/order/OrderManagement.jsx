...
const OrderManagement = () => {
  ...
  const columns = [
    ...
    // Thêm cột tổng tiền vào cấu hình columns - thêm vào trước cột "Thao tác"
    {
      ...
      render: (_, record) => {
        ...
        const subtotal =
          record.orderItems?.reduce((total, item) => total + item.priceAtOrderTime * item.quantity, 0) || 0;
  ...
  const exportInvoiceToPdf = (order) => {
    ...
    doc.text("Địa chỉ: 41A Đ. Phú Diễn, Phú Diễn, Bắc Từ Liêm, Hà Nội", 105, 35, { align: "center" });
      ...
      <div style={{ padding: 24, background: "#fff" }}>
        ...
        >
          <div style={{ display: "flex", gap: "16px", alignItems: "center" }}>
            ...
            <Select value={sortField} onChange={handleSortFieldChange} style={{ width: 180 }} size="middle">
            ...
            <>
              <Descriptions title="Thông tin đơn hàng" bordered>
                ...
                <Descriptions.Item label="Cập nhật lần cuối" span={2}>
                  {currentOrder.updatedAt ? dayjs(currentOrder.updatedAt).format("DD/MM/YYYY HH:mm") : "Chưa cập nhật"}
                </Descriptions.Item>
              ...
              <Row gutter={16}>
                <Col span={12}>
                  <Card title="Thông tin người mua" bordered={false} style={{ height: "100%" }}>
                    <p>
                      <strong>Tên:</strong> {currentOrder.user?.fullName || "N/A"}
                    </p>
                  </Card>
                </Col>
                ...
                <Col span={12}>
                  <Card title="Thông tin người nhận" bordered={false} style={{ height: "100%" }}>
                    <p>
                      <strong>Tên:</strong> {currentOrder.infoUserReceive?.fullName || "N/A"}
                    </p>
                    <p>
                      <strong>Email:</strong> {currentOrder.infoUserReceive?.email || "N/A"}
                    </p>
                    <p>
                      <strong>SĐT:</strong> {currentOrder.infoUserReceive?.phoneNumber || "N/A"}
                    </p>
                    <p>
                      <strong>Địa chỉ:</strong> {formatAddress(currentOrder.infoUserReceive)}
                    </p>
                  </Card>
                </Col>
              </Row>
        ...
        <Modal
          ...
          footer={[
            ...
            <Button key="submit" type="primary" loading={loading} onClick={handleStatusUpdate}>
              Cập nhật
            </Button>,
          ]}
        >
        ...
        <Modal
          ...
          footer={[
            ...
            <Button key="submit" type="primary" loading={loading} onClick={handleAcceptRefund}>
              Xác nhận hoàn tiền
            </Button>,
          ]}
        >
          ...
          <p>Hành động này sẽ hoàn tiền cho khách hàng và không thể hoàn tác. Bạn có chắc chắn muốn tiếp tục?</p>

          {orderToRefund && (
            <div style={{ background: "#f5f5f5", padding: "12px", borderRadius: "4px", marginTop: "16px" }}>
              <p style={{ margin: "0 0 8px 0" }}>
                <strong>Khách hàng:</strong> {orderToRefund.user?.fullName}
              </p>
              <p style={{ margin: "0 0 8px 0" }}>
                ...
                <span style={{ color: "#ff4d4f", fontWeight: "bold" }}>
                  {new Intl.NumberFormat("vi-VN", {
                    ...
                    orderToRefund.orderItems?.reduce(
                      (total, item) => total + item.priceAtOrderTime * item.quantity,
                      0
